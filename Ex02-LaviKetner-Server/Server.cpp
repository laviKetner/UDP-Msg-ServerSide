...
void getDayOfYear(char *sendBuff);
...
int main()
{
	...
	while (true)
	{
		...
		cout << "Time Server: Recieved: " << bytesRecv << " bytes of \"" << (int)recvBuff[0] << "\" message.\n";
...
void getTime(char *sendBuff)
{
	...
	strcpy(sendBuff, ctime(&currentTime));
...
void getTimeWithoutDate(char *sendBuff)
{
	...
	strftime(sendBuff, 255, "%H:%M:%S", timeInfo);
}

void getTimeSinceEpoch(char *sendBuff)
{
	time_t secondsFrom1970;
	secondsFrom1970 = time(NULL);
	sprintf(sendBuff, "%d", (int)secondsFrom1970);
}
...
void getTimeWithoutDateOrSeconds(char *sendBuff)
{
	...
	timeInfo = localtime(&currentTime);
	strftime(sendBuff, 255, "%H:%M:--", timeInfo);
}
...
void getSecondsSinceBeginingOfMonth(char *sendBuff)
{
	...
	double seconds = difftime(nowTime, mktime(&timeInfo));
	sprintf(sendBuff, "%d", (int)seconds);
}
...
void getDayOfYear(char *sendBuff)
{
	...
	sprintf(sendBuff, "%d", (int)timeInfo.tm_yday);
}
...
void getDaylightSavings(char *sendBuff)
{
	...
	struct tm timeInfo;
	timeInfo = *localtime(&nowTime);
	sprintf(sendBuff, "%d", (int)timeInfo.tm_isdst);
}

void getClientToServerDelayEstimation(char *sendBuff)
{
	DWORD nowTime = GetTickCount();
	sprintf(sendBuff, "%d", (int)nowTime);
}
